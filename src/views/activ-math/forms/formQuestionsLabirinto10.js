import { Row, Col, Label, Input, Button, Form } from 'reactstrap';
import { useState } from 'react';
import axios from 'axios';
import { toast } from 'react-toastify';
import { ErrorToast, SuccessToast } from '../../components/toasts/Error';

const QuestionsLabirinto10 = () => {
  const [data, setData] = useState({
    pergunta1: '',
    resposta1certa: '',
    resposta1errada: '',
    pergunta2: '',
    resposta2certa: '',
    resposta2errada: '',
    pergunta3: '',
    resposta3certa: '',
    resposta3errada: '',
    pergunta4: '',
    resposta4certa: '',
    resposta4errada: '',
    pergunta5: '',
    resposta5certa: '',
    resposta5errada: '',
    pergunta6: '',
    resposta6certa: '',
    resposta6errada: '',
    pergunta7: '',
    resposta7certa: '',
    resposta7errada: '',
    pergunta8: '',
    resposta8certa: '',
    resposta8errada: '',
    pergunta9: '',
    resposta9certa: '',
    resposta9errada: '',
    pergunta10: '',
    resposta10certa: '',
    resposta10errada: '',
  });

  function handle(e) {
    const newdata = { ...data };
    newdata[e.target.id] = e.target.value;
    setData(newdata);
    console.log(newdata);
  }

  async function submit(e) {
    e.preventDefault();
    try {
      await axios.post('https://sky-escola-api.herokuapp.com/activities', {
        teacher_id: '62968757997e5340bbe8d101',
        questions: {
          labirinto: {
            pergunta1: data.pergunta1,
            resposta1certa: data.resposta1certa,
            resposta1errada: data.resposta1errada,
            pergunta2: data.pergunta2,
            resposta2certa: data.resposta2certa,
            resposta2errada: data.resposta2errada,
            pergunta3: data.pergunta3,
            resposta3certa: data.resposta3certa,
            resposta3errada: data.resposta3errada,
            pergunta4: data.pergunta4,
            resposta4certa: data.resposta4certa,
            resposta4errada: data.resposta4errada,
            pergunta5: data.pergunta5,
            resposta5certa: data.resposta5certa,
            resposta5errada: data.resposta5errada,
            pergunta6: data.pergunta6,
            resposta6certa: data.resposta6certa,
            resposta6errada: data.resposta6errada,
            pergunta7: data.pergunta7,
            resposta7certa: data.resposta7certa,
            resposta7errada: data.resposta7errada,
            pergunta8: data.pergunta8,
            resposta8certa: data.resposta8certa,
            resposta8errada: data.resposta8errada,
            pergunta9: data.pergunta9,
            resposta9certa: data.resposta9certa,
            resposta9errada: data.resposta9errada,
            pergunta10: data.pergunta10,
            resposta10certa: data.resposta10certa,
            resposta10errada: data.resposta10errada,
          },
        },
      });
      toast.success(
        <SuccessToast description="Questions upload was successfully" />,
        {
          icon: false,
          hideProgressBar: true,
        }
      );
    } catch (error) {
      console.log(error);
      toast.error(<ErrorToast description="Error, there was an error " />, {
        icon: false,
        hideProgressBar: true,
      });
    }
  }

  return (
    <>
      <Form onSubmit={(e) => submit(e)}>
        <Row className="match-height mt-3">
          <Col xl="12" md="12" xs="12">
            <Row className="match-height">
              <Col xl="4" md="4" xs="12">
                <div className="mb-2">
                  <Label className="form-label" for="pergunta1">
                    Pergunta 1:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.pergunta1}
                    id="pergunta1"
                    type="text"
                    placeholder="Digite a 1 pergunta..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-success"
                    for="resposta1certa"
                  >
                    Resposta 1 Certa:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta1certa}
                    type="text"
                    id="resposta1certa"
                    placeholder="Digite a 1 resposta certa..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-danger"
                    for="resposta1errada"
                  >
                    Resposta 1 Errada:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta1errada}
                    type="text"
                    id="resposta1errada"
                    placeholder="Digite a 1 resposta errada..."
                  />
                </div>
              </Col>
            </Row>
            <hr />
            <Row className="match-height">
              <Col xl="4" md="4" xs="12">
                <div className="mb-2">
                  <Label className="form-label" for="pergunta2">
                    Pergunta 2:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.pergunta2}
                    type="text"
                    id="pergunta2"
                    placeholder="Digite a 2 pergunta..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-success"
                    for="resposta2certa"
                  >
                    Resposta 2 Certa:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta2certa}
                    type="text"
                    id="resposta2certa"
                    placeholder="Digite a 2 resposta certa..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-danger"
                    for="resposta2errada"
                  >
                    Resposta 2 Errada:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta2errada}
                    type="text"
                    id="resposta2errada"
                    placeholder="Digite a 2 resposta errada..."
                  />
                </div>
              </Col>
            </Row>
            <hr />
            <Row className="match-height">
              <Col xl="4" md="4" xs="12">
                <div className="mb-2">
                  <Label className="form-label" for="pergunta3">
                    Pergunta 3:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.pergunta3}
                    type="text"
                    id="pergunta3"
                    placeholder="Digite a 3 pergunta..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-success"
                    for="resposta3certa"
                  >
                    Resposta 3 Certa:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta3certa}
                    type="text"
                    id="resposta3certa"
                    placeholder="Digite a 3 resposta certa..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-danger"
                    for="resposta3errada"
                  >
                    Resposta 3 Errada:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta3errada}
                    type="text"
                    id="resposta3errada"
                    placeholder="Digite a 3 resposta errada..."
                  />
                </div>
              </Col>
            </Row>
            <Row className="match-height">
              <Col xl="4" md="4" xs="12">
                <div className="mb-2">
                  <Label className="form-label" for="pergunta4">
                    Pergunta 4:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.pergunta4}
                    type="text"
                    id="pergunta4"
                    placeholder="Digite a 4 pergunta..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-success"
                    for="resposta4certa"
                  >
                    Resposta 4 Certa:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta4certa}
                    type="text"
                    id="resposta4certa"
                    placeholder="Digite a 4 resposta certa..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-danger"
                    for="resposta4errada"
                  >
                    Resposta 4 Errada:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta4errada}
                    type="text"
                    id="resposta4errada"
                    placeholder="Digite a 4 resposta errada..."
                  />
                </div>
              </Col>
            </Row>
            <Row className="match-height">
              <Col xl="4" md="4" xs="12">
                <div className="mb-2">
                  <Label className="form-label" for="pergunta5">
                    Pergunta 5:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.pergunta5}
                    type="text"
                    id="pergunta5"
                    placeholder="Digite a 5 pergunta..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-success"
                    for="resposta5certa"
                  >
                    Resposta 5 Certa:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta5certa}
                    type="text"
                    id="resposta5certa"
                    placeholder="Digite a 5 resposta certa..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-danger"
                    for="resposta5errada"
                  >
                    Resposta 5 Errada:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta5errada}
                    type="text"
                    id="resposta5errada"
                    placeholder="Digite a 5 resposta errada..."
                  />
                </div>
              </Col>
            </Row>
            <Row className="match-height">
              <Col xl="4" md="4" xs="12">
                <div className="mb-2">
                  <Label className="form-label" for="pergunta6">
                    Pergunta 6:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.pergunta6}
                    type="text"
                    id="pergunta6"
                    placeholder="Digite a 6 pergunta..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-success"
                    for="resposta6certa"
                  >
                    Resposta 6 Certa:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta6certa}
                    type="text"
                    id="resposta6certa"
                    placeholder="Digite a 6 resposta certa..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-danger"
                    for="resposta6errada"
                  >
                    Resposta 6 Errada:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta6errada}
                    type="text"
                    id="resposta6errada"
                    placeholder="Digite a 6 resposta errada..."
                  />
                </div>
              </Col>
            </Row>
            <Row className="match-height">
              <Col xl="4" md="4" xs="12">
                <div className="mb-2">
                  <Label className="form-label" for="pergunta7">
                    Pergunta 7:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.pergunta7}
                    type="text"
                    id="pergunta7"
                    placeholder="Digite a 7 pergunta..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-success"
                    for="resposta7certa"
                  >
                    Resposta 7 Certa:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta7certa}
                    type="text"
                    id="resposta7certa"
                    placeholder="Digite a 7 resposta certa..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-danger"
                    for="resposta7errada"
                  >
                    Resposta 7 Errada:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta7errada}
                    type="text"
                    id="resposta7errada"
                    placeholder="Digite a 7 resposta errada..."
                  />
                </div>
              </Col>
            </Row>
            <Row className="match-height">
              <Col xl="4" md="4" xs="12">
                <div className="mb-2">
                  <Label className="form-label" for="pergunta8">
                    Pergunta 8:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.pergunta8}
                    type="text"
                    id="pergunta8"
                    placeholder="Digite a 8 pergunta..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-success"
                    for="resposta8certa"
                  >
                    Resposta 8 Certa:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta8certa}
                    type="text"
                    id="resposta8certa"
                    placeholder="Digite a 8 resposta certa..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-danger"
                    for="resposta8errada"
                  >
                    Resposta 8 Errada:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta8errada}
                    type="text"
                    id="resposta8errada"
                    placeholder="Digite a 8 resposta errada..."
                  />
                </div>
              </Col>
            </Row>
            <Row className="match-height">
              <Col xl="4" md="4" xs="12">
                <div className="mb-2">
                  <Label className="form-label" for="pergunta9">
                    Pergunta 9:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.pergunta9}
                    type="text"
                    id="pergunta9"
                    placeholder="Digite a 9 pergunta..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-success"
                    for="resposta9certa"
                  >
                    Resposta 9 Certa:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta9certa}
                    type="text"
                    id="resposta9certa"
                    placeholder="Digite a 9 resposta certa..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-danger"
                    for="resposta9errada"
                  >
                    Resposta 9 Errada:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta9errada}
                    type="text"
                    id="resposta9errada"
                    placeholder="Digite a 9 resposta errada..."
                  />
                </div>
              </Col>
            </Row>
            <Row className="match-height">
              <Col xl="4" md="4" xs="12">
                <div className="mb-2">
                  <Label className="form-label" for="pergunta10">
                    Pergunta 10:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.pergunta10}
                    type="text"
                    id="pergunta10"
                    placeholder="Digite a 10 pergunta..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-success"
                    for="resposta10certa"
                  >
                    Resposta 10 Certa:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta10certa}
                    type="text"
                    id="resposta10certa"
                    placeholder="Digite a 10 resposta certa..."
                  />
                </div>
              </Col>
              <Col xl="4" md="4" xs="6">
                <div className="mb-2">
                  <Label
                    className="form-label text-danger"
                    for="resposta10errada"
                  >
                    Resposta 10 Errada:
                  </Label>
                  <Input
                    onChange={(e) => handle(e)}
                    value={data.resposta10errada}
                    type="text"
                    id="resposta10errada"
                    placeholder="Digite a 10 resposta errada..."
                  />
                </div>
              </Col>
            </Row>
            <Button color="success" type="submit">
              Concluir
            </Button>
          </Col>
        </Row>
      </Form>
    </>
  );
};
export default QuestionsLabirinto10;
