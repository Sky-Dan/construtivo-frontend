  ...
  const percentages = uniqueLabels
    .map((category) => {
      const transactions = store.transactions.filter(
        (t) => t.category === category && t.kind === 'expense'
      );
      return transactions;
    })
    .map((t) => {
      ...
      return Number(((totalCategory * 100) / store.totalExpense).toFixed(2));
    });
...
export default ApexRadiarChartExpenses;
